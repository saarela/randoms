...
library(e1071)  ## as the name suggests, this is needed for SVM
...
plot_contours <- function(mu,d,npoints,x) {
    ...
    levels <- dmnorm(matrix(c(0,0,1,0,2,0),ncol=2,byrow=T),
                     c(0,0),
...
## max value of the figure axes
xmax <- 3

## Set class means.  The space is two-dimensional so the matrix has
## two columns.  Each row of this Nx2 matrix is a mean of a single
## class; there are as many rows as there are classes.
mu <- matrix(c(-0,-1,
                2, 1,
               -1, 1),
             ncol=2,byrow=TRUE)
...
## Covariance matrices.  Make one 2x2xN array to hold all the
## covariance matrices.  Each "slice" along the third dimension gives
## the covariance matrix for a single class.  First initialize:
Sigma <- array(NA,dim=c(2,2,nclass))

## Then set the values.  Here, all covariance matrices are equal:
Sigma[,,1] <- c(1,0,0,1)
Sigma[,,2] <- c(1,0,0,1)
Sigma[,,3] <- c(1,0,0,1)

## Or use unequal covariance matrices:
## Sigma[,,1] <- c(.3,0,0,.3)
## Sigma[,,2] <- c(1.3,.6,.6,.6)
## Sigma[,,3] <- c(1.5,0,0,1.5)

## Make random data vectors.  First initialize the matrix:
x <- matrix(NA,ncol=2,nrow=n)
## Then loop through the classes and make random draws from the
## appropriate multivariate normal distribution:
for (ii in 1:nclass) 
    x[cl==ii-1,] <- rmnorm(sum(cl==ii-1),mu[ii,],Sigma[,,ii])
...
d <- matrix(NA,ncol=dim(mu)[1],nrow=dim(dat.bkgr)[1])
for (ii in 1:nclass) {
    d[,ii] <- dmnorm(dat.bkgr,mu[ii,],Sigma[,,ii])
}
...
l <- matrix(NA,ncol=dim(mu)[1],nrow=dim(dat.test)[1])
for (ii in 1:nclass) {
    l[,ii] <- dmnorm(dat.test[,c("x1","x2")],mu[ii,],Sigma[,,ii])
}
...
## OR: pc.svmlin <- mean(clhat.svmlin==dat.test$class)

print(conf.svmlin)
cat("SVM prop. correct: ",pc.svmlin,"\n",sep="")
...
## Train
m.svm <- svm(class~x1+x2,data=dat.train)
...
## Get the predicted classes for the test data given the training data
## and training data labels
...
## Plot test data
plot_classification(dat.test,clhat.nvb,
                    sprintf("Accuracy: %4.2f",pc.nvb),expression(x[1]),"")
